...
import { Ban, Delete, Edit2Icon, MessageCircle, MoreHorizontal, Share2, ThumbsUp, Trash2 } from "lucide-react";
...
function PostCard({ posts, isliked, onLike, onComment, onShare }) {
  ...
  const userPlaceholder = posts?.user?.username
    ?.split(" ")
    .map((name) => name[0])
    .join("");
  ...
  useEffect(() => {
    ...
    (async() => {
      if (posts?.user?._id) {
        const result = await fetchUserProfile(posts?.user?._id);
  ...
  const handleDelte = async () => {
    ...
    setLoading(true)
    await deletePost(posts?._id, formData);
    setOpenDelete(false);
    await fetchPost();
      ...
      <Card className="mt-2 ">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mt-4">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => {router.push(`/userProfile/${posts?.user?._id}`)}}>
              <Avatar>
          ...
          {posts?.mediaUrl && posts.mediaType === "video" && (
            <video controls muted className="w-full h-[500px] rounded-lg mb-4">
          ...
          <div className="flex justify-between mb-2 ">
            ...
            <Button
              variant="ghost"
              className={`flex-1 dark:hover:bg-gray-600 dark:text-gray-200 `}
            ...
            >
              <DialogTrigger asChild>
                <Button
                  variant="ghost"
                  className="flex-1 dark:hover:bg-gray-500 dark:text-gray-200"
                  onClick={onShare}
                >
